...
Node* insertNode(Node *head, int i, int data) {
    ...
    {
          if(i==0){//head null hai but position zero hai matlab mujhe first pos pe node dalna hai or agar pos zero nahi hai matlab
        Node *newNode= new Node(data);// mujhe is loop mein jana hi  nahi hai simply head return mar do
    ...
    head->next = insertNode(head->next,i-1,data);//yahan pe insertNode function return karega apna head or taki woh previous wala head is naye head ko point kare isliye retened head ko head->next mai daal rahe hain
    return head;
}
